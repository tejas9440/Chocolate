...
export default function MyCart(props) {
    ...
    const handleIncrement = async (productId, index) => {
        const userId = localStorage.getItem('userId')
        try {
            let response = await fetch(`https://chocolate-s86f.onrender.com/quantity`, {
                method: 'POST',
    ...
    const handleDecrement = async (productId, index) => {
        const userId = localStorage.getItem('userId')
        try {
            const response = await axios.post('https://chocolate-s86f.onrender.com/quantity', {
                userId,
                productId,
                action: 'dec',
            });
    ...
    const handleRemove = async (productId, index) => {
        const userId = localStorage.getItem('userId')
        try {
            await axios.post('https://chocolate-s86f.onrender.com/remove-from-cart', {
                userId,
                productId,
            });
